...
// Function to draw a simple square, pointing towards z, with the center at pos, and size of 10 units,
// along with a texture and color, along with a scale factor for the size of the square
void DrawSquare(const float *pos, GRRLIB_texImg *tex, unsigned int color, float scaleX, float scaleY)
{
    float half_size = 5.0f;

    float hw = half_size * scaleX;
    float hh = half_size * scaleY;

    float vertices[4][3] = {
        {pos[0] - hw, pos[1] - hh, pos[2]},
        {pos[0] + hw, pos[1] - hh, pos[2]},
        {pos[0] + hw, pos[1] + hh, pos[2]},
        {pos[0] - hw, pos[1] + hh, pos[2]},
    };
    ...
    for (int i = 0; i < 4; i++)
    {
        GX_Position3f32(vertices[i][0], vertices[i][1], vertices[i][2]);
        GX_Color1u32(color);
        GX_TexCoord2f32(uv[i][0], uv[i][1]);
    }
    GX_End();
}
