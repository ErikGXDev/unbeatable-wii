...
let outContent = "// Automatically generated by scripts/build_objs.ts\n\n";
...
for (const file of files) {
  ...
  if (isMarker) {
    ...
    outContent += "// " + file + "\n";
    outContent += `const float ${objData.name}_position[3] = { ${position.join(", ")} };\n\n`;
  ...
  outContent += "// " + file + "\n";
  outContent += `const int ${objData.name}_vertex_count = ${finalVertices.length};\n\n`;

  outContent += `const float ${objData.name}_vertices[${finalVertices.length}][8] = {\n`;
  for (const vertex of finalVertices) {
    outContent += `  { ${vertex.join(", ")} },\n`;
  }
  outContent += "};\n";
  ...
  if (currentMaterial) {
    ...
    if (textureData) {
      ...
      if (textureData) {
        outContent += `// Texture: ${textureData.path}\n`;
      }
      outContent += "const int " + objData.name + "_texture = " + index + ";\n";
    }
  }
...
// List textures
for (const textureData of allTextureData) {
  outContent += `#include "${textureData.includePath}"\n`;
}

outContent +=
  "const int exported_texture_count = " + allTextureData.length + ";\n";
outContent += "const uint8_t* exported_textures[] = {\n";
for (const textureData of allTextureData) {
  outContent += ` (const uint8_t*)${textureData.variableName},\n`;
}
outContent += "};\n\n";
...
outContent += "const int exported_objects_texture_indices[] = {\n";
...
outContent += "const int exported_objects_vertex_counts[] = {\n";
for (const objData of allObjData) {
  outContent += `  ${objData.name}_vertex_count,\n`;
}
outContent += "};\n\n";

outContent += "const float* exported_objects_vertices[] = {\n";
for (const objData of allObjData) {
  outContent += `  (const float*)${objData.name}_vertices,\n`;
}
outContent += "};\n";

outContent += "\n";
outContent += "const int exported_num_objects = " + allObjData.length + ";\n";
